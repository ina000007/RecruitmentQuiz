...
export class CreateTestComponent implements OnInit {
  ...
  constructor(private testSetService:TestSetService,private clgService:CollgeService,
    ...
    calEndTime(){
      // alert("here");
      // alert("here "+this.allocatedTimeForTest+" "+this.startTimeForTest+" "+this.endTimeForTest);
      var mn= this.allocatedTimeForTest%60;
      var hr= (Math.floor)(this.allocatedTimeForTest/60);
      var tmp= this.startTimeForTest.split(":");
      mn  =  mn + parseInt(tmp[1]);
      if(mn>59)
      hr = hr + parseInt(tmp[0])+1;
      else
      hr = hr + parseInt(tmp[0]);
      mn = mn%60; 
      this.endTimeForTest = Utils.pad(hr)+":"+Utils.pad(mn);
